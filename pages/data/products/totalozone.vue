...
export default {
  ...
  async mounted() {
    ...
    const stationsRaw = this.$store.getters['stations/totalozone'].name
  ...
  methods: {
    ...
    reorderStations() {
      let stationsRaw
      if (this.stationOrderByID) {
        stationsRaw = this.$store.getters['stations/totalozone'].id
      } else {
        stationsRaw = this.$store.getters['stations/totalozone'].name
      }
